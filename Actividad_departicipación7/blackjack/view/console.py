...
class UIConsola:
    ...
    def ejecutar_turno_de_la_casa(self):
        ...
        if self.blackjack.casa_perdio(): 
            print("¡FELICIDADES, HAS GANADO! La casa ha superado los 21.") 
        else: 
            if self.blackjack.cupier.mano.calcular_valor() > self.blackjack.jugador.mano.calcular_valor():
               print("LA CASA HA GANADO.") 
            elif self.blackjack.cupier.mano.calcular_valor() < self.blackjack.jugador.mano.calcular_valor():
    ...
    def finalizar_juego(self): 
        ...
        if self.blackjack.casa_gano():
            print("EL JUGADOR HA PERDIDO EL JUEGO") 
